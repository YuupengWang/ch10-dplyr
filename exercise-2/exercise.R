...
worest <- select(filter(two_wheel_drive, hwy == min(hwy)), id)

# Write a function that takes a `year_choice` and a `make_choice` as parameters,
# and returns the vehicle model that gets the most hwy miles/gallon of vehicles 
# of that make in that year.
# You'll need to filter more (and do some selecting)!
best_vehicle <- function(yeara, makes) {
  time <- filter(vehicles, year == yeara, make == makes)
  best <- select(filter(time, hwy == max(hwy)), model)
  return(best)
}

# What was the most efficient Honda model of 1995?
efficient <- best_vehicle(1995, 'Honda')
